...
class AStar2 {
    ...
    public search():boolean{
        ...
        while(!this.isEnd){
            ...
            var aroundNodeX = new Array<number>(0 > node.x - 1 ? 0 : node.x - 1,node.x,node.x,this._grid.numCols - 1 < node.x + 1 ? this._grid.numCols - 1 : node.x + 1);
            var aroundNodeY = new Array<number>(node.y,0 > node.y - 1 ? 0 : node.y - 1,this._grid.numRows - 1 < node.y + 1 ? this._grid.numRows - 1 : node.y + 1,node.y);
            ////一次九宫格循环节点
            for(var i:number = 0; i < 4; i++){
                //for(var j:number = startY; j <= endY; j++){
                    ////当前要被探查的节点
                    var test:NodePoint = this._grid.getNode(aroundNodeX[i],aroundNodeY[i]);
